...
df <- read.csv(here("data/csv/final_estimates_block_groups_only_wells.csv"))%>%
...
ggplot(long)+
  ...
  coord_flip()+
  labs(x = "", y = "Housing Units Using Private Wells (Millions)", title = "Est. Housing Units Using Wells\n by State & Method")+
...
# Crosshatch
ggplot(long)+
  geom_col(aes(x = reorder(State,Total_NHU), y = Total/1000000, fill = Method),position = "dodge")+
  geom_col(data = long[long$Method == "RW", ], aes(x = reorder(State,Total_NHU), y = Total/1000000, fill = Method),width = 1.2, position = position_dodge2(width = .9, padding = 0))+
  coord_flip()+
  labs(x = "", y = "Housing Units Using Private Wells (Millions)", title = "Est. Housing Units Using Wells\n by State & Method")+
  scale_fill_manual(values = c("#00cc33","#3399ff"))+
  scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE))+
...
ggplot()+
  geom_col(data = long[long$Method == "NHU", ], aes(x = reorder(State,Total_NHU), y = Total/1000000, fill = Method), position = "dodge")+
  geom_col(data = long[long$Method == "RW", ], aes(x = reorder(State,Total_NHU), y = Total/1000000, fill = Method), width = 0.9, position = position_dodge(width = 0.9))+
  coord_flip()+
  labs(x = "", y = "Housing Units Using Private Wells (Millions)", title = "Est. Housing Units Using Wells\n by State & Method")+
  scale_fill_manual(values = c("#00cc33","#3399ff"))+
  scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE))+
